...
public class ItemMenuWeapons implements InventoryHolder {
    private static Inventory inv;
    public ItemMenuWeapons(){
        int num = (int) Math.floor(Weapons.items.size()/9)*9;
        if(Math.floor(Weapons.items.size()/9)*9==0){
            num = 9;
        }
        inv = Bukkit.createInventory(this,num+9, "Weapons");
        init(num+9);
    }
    private static void init(int num){
        for(int i = 0; i < num; i++){
            if(i>num-10){
            ...
            }else if(Weapons.items.size()-1>=i){
                ItemStack item = Weapons.items.get(i).clone();
                ...
                lore.add(" ");
                lore.add(ChatColor.DARK_BLUE+"ID: "+Weapons.items.indexOf(item));
                lore.add(" ");
