...
public class ItemMenuItems implements InventoryHolder {
    private static Inventory inv;
    public ItemMenuItems(){
        int num = (int) Math.ceil((double)Items.items.size()/9)*9;
        if(Math.ceil((double)Items.items.size()/9)*9==0){
            num = 9;
        }
    ...
    private static void init(int num){
        for(int i = 0; i < num; i++){
            if(i>num-10){
            ...
            }else if(Items.items.size()-1>=i){
                ItemStack item = Items.items.get(i).clone();
                ...
                lore.add(" ");
                lore.add(ChatColor.DARK_BLUE+"ID: "+ Items.items.indexOf(item));
                lore.add(" ");
